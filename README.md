...
- **Linux/macOS**: No additional setup required _(except `zip` if not installed)._
...
# Using WinRAR for Compression in Git Bash on Windows

If you are using **Git Bash** on Windows and cannot use `sudo apt install zip`, you can use **WinRAR** to compress files instead. Follow these steps:
...
## 2. Add WinRAR to the System Path

- Press **Win + R**, type `sysdm.cpl`, and press **Enter**.
- Go to the **Advanced** tab and click **Environment Variables**.
- Under **System Variables**, find and select **Path**, then click **Edit**.
- Click **New**, then paste the following path (assuming you installed WinRAR in the default location):  
  `C:\Program Files\WinRAR`
- Click **OK** to save and close the windows.
- Restart **Git Bash** to apply the changes.

## 3. Modify the `compress.sh` Script

- Open the `compress.sh` script.
- Go to **line 20**, where it has the following code:
...
```sh
winrar a -afzip "$zipname.zip" "$source"
```
